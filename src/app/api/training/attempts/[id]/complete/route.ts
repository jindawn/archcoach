...
import { getAttempt, completeAttempt } from "@/db/repositories/guidedTraining";
...
export async function POST(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (found.attempt.submissionId) return ok({ attempt: found.attempt, submissionId: found.attempt.submissionId });
    ...
    const attempt = await completeAttempt(found.attempt.id, { submissionId: submission.id, independenceScore, capabilityScores: assessment, recommendedStepId: recommendedStep(guide, assessment, hintMap) });
    return ok({ attempt, submissionId: submission.id }, 201);
