...
import { z } from "zod";
import { getAttempt, getLatestAnswer, saveStepAnswer, setFollowUp, setHintLevel, updateAssessment } from "@/db/repositories/guidedTraining";
import { getScenarioBySlug } from "@/db/repositories/scenarios";
import { assessAnswers, generateFollowUp, parseGuide, recommendedStep } from "@/lib/guided-training";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const input = z.discriminatedUnion("action", [
      ...
      z.object({ action: z.literal("draft"), stepId: z.string(), answer: z.string().max(8000), followUpAnswer: z.string().max(4000).optional() }),
    ...
    if (input.action === "save" && !row.followUpQuestion) { const question = await generateFollowUp(found.guide, input.stepId, input.answer); await setFollowUp(row.id, question); row.followUpQuestion = question; }
    if (input.action === "retry") {
      ...
      await updateAssessment(id, { capabilityScores: assessment, recommendedStepId: recommendedStep(found.guide, assessment, hintMap) });
    }
    return ok(row);
