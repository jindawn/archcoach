...
export function TeamSettings({ teamId, isOwner }: { teamId: string; isOwner: boolean }) {
  ...
  const invite = async () => { setError(null); setNotice(null); const response = await fetch(`/api/teams/${teamId}/invitations`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, role: "member" }) }); const body = await response.json(); if (!body.success) return setError(body.error); setInviteUrl(body.data.inviteUrl); setNotice(body.data.emailSent ? "邀请邮件已发送，同时可复制下方链接。" : "未配置邮件服务或发送失败，请复制下方链接。" ); setEmail(""); };
