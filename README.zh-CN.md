...
- **已完成** — 多用户认证、GitHub OAuth、10 角色委员会、训练推荐、持久化评审 worker、团队评审室与团队知识检索
- **已完成** — owner/member 角色与绑定邮箱、七天有效的团队邀请链接
- **已完成** — 可选 OpenAI 兼容语义向量检索，并在未配置时回退词法检索
- **已完成** — 可选 Resend 邀请邮件发送，并保留可复制邀请链接作为回退
